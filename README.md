<img src="https://capsule-render.vercel.app/api?type=soft&color=edf1f7&height=230&section=header&text=mite88&fontAlignY=45&fontSize=82&fontColor=568bdb&animation=blink&desc=Backend%20%7C%20Spring%20%7C%20Service%20Flow%20Developer&descSize=24&descAlignY=73">

<br>

# 안녕하세요, 전옥주입니다.

6년차 Java/Spring 기반 백엔드 개발자 전옥주입니다. 
새로운 기술을 활용해 비즈니스 문제를 해결하는 것에 관심이 많습니다. 
공공기관 SI와 이커머스 서비스의 백엔드 설계·개발·운영 경험, 10년 이상 운영된 PHP 레거시를 Spring Boot로 전환 
한 경험으로 안정적인 시스템 설계와 SAP·AI 등 이기종 시스템 연동에 대한 높은 이해도를 가지고 있으며, 기획부터 
배포까지 독립적으로 수행해 온 주도성 및 다양한 팀과의 협업 경험으로 원활한 커뮤니케이션이 가능합니다.

<br>

## 🛠️ &nbsp;TECH STACKS

### Backend

![Java](https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![JPA](https://img.shields.io/badge/JPA-59666C?style=for-the-badge&logo=hibernate&logoColor=white)
![MyBatis](https://img.shields.io/badge/MyBatis-111111?style=for-the-badge&logoColor=white)
![eGovFrame](https://img.shields.io/badge/eGovFrame-005BAC?style=for-the-badge&logoColor=white)
![JSP](https://img.shields.io/badge/JSP-007396?style=for-the-badge&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)

### DB · Infra

![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![AWS EC2](https://img.shields.io/badge/AWS%20EC2-FF9900?style=for-the-badge&logo=amazonec2&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### AI

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge&logoColor=white)
![Deep Learning](https://img.shields.io/badge/Deep%20Learning-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-6D28D9?style=for-the-badge&logoColor=white)
![Chatbot](https://img.shields.io/badge/Chatbot-00A67E?style=for-the-badge&logo=openai&logoColor=white)

<br>


## 🎁 &nbsp;PROJECTS

<h3>🛒 &nbsp;주문·결제 검증과 관리자 운영 기능을 구현한 이커머스 서비스 <b>"SnackDeal"</b></h3>

<div markdown="1">

- 진행 기간: 2026.06 - 2026.07
- 참여 인원: 3명
- 기술 스택: Java, Spring Boot, JPA, MySQL, Redis, JWT, PortOne/Toss
- 담당 기능: 회원·이메일 인증, JWT 로그인/재발급, 장바구니, 주문 준비, 결제 검증/확정, 환불, 관리자 회원/주문 관리
- 핵심 구현: 서버에서 상품 가격과 수량 기준으로 주문 금액을 재계산하고 결제 승인 금액과 비교해 결제 조작 가능성을 줄였습니다.
- GitHub: https://github.com/orgs/SnackDeal
- 배포 링크: https://snackdeal.mite88.site

</div>

<br>

<h3>💬 &nbsp;PHP 레거시를 Spring Boot 구조로 전환한 멀티도메인 BERT 챗봇 <b>"중소기업 기술혁신 R&D"</b></h3>

<div markdown="1">

- 진행 기간: 2022.09 - 2024.07
- 참여 인원: 2명
- 기술 스택: Java, Spring Boot, JPA, BERT, Python, PDF, REST API
- 담당 기능: PHP 레거시 → Spring Boot 마이그레이션, 챗봇 API 연동, Python 분석 결과 변환, 화면 출력, PDF 보고서 생성
- 핵심 구현: 단일 도메인 중심의 챗봇 기능을 멀티도메인 구조로 확장하고, 분석 결과를 서비스 화면과 PDF 보고서로 연결했습니다.

</div>

<br>

<h3>🎮 &nbsp;Steam 인증과 레이드 결과 정합성을 설계 중인 게임 백엔드 <b>"Frontier"</b></h3>

<div markdown="1">

- 진행 상태: 현재 개발 중
- 참여 인원: 확인 필요
- 기술 스택: Java, Spring Boot, JWT, Steam Web API, Transaction, Optimistic Lock
- 담당 기능: Steam Ticket 인증, Access/Refresh JWT, Dedicated Server Service JWT, 레이드 결과 정산 API 설계
- 핵심 설계: 전투 중 상태는 Dedicated Server가 관리하고, 사망·탈출 시 최종 결과만 Backend에서 트랜잭션으로 확정하는 구조를 설계하고 있습니다.

</div>

<br>

## 📌 &nbsp;OTHER PROJECTS

<h4>서울교통공사 챗봇 연동 | SAP 업무 프로세스 연동</h4>

- 2022.12 - 2023.12 · Spring Boot, SAP RFC/JCo, Oracle, REST API
- 챗봇 REST API와 SAP 인사·행정 데이터를 중간 API에서 변환·연동

<h4>종로사장 / 종로 뉴미디어 | 공공 웹·관리자 기능 개발</h4>

- 2023.11 - 2025.09 · Spring Boot, eGovFrame, JSP, MySQL, Map API, SMS API
- 관리자, 본인인증/결제, SMS 발송 이력, 지도 시각화 기능 구현

<h4>이커머스 웹 구축·운영 | Cafe24 쇼핑몰·기업 사이트</h4>

- 2018.11 - 2022.02 · Cafe24, PHP, JavaScript, MySQL/MariaDB
- 요구사항 확인부터 화면/기능 구현, 배포, 운영 이슈 대응까지 수행

<h4>GBDT 정책 스크리닝 | 정형 데이터 기반 ML</h4>

- AI 교육 프로젝트 · Python, 데이터 전처리, GBDT, 모델 평가
- 정책 데이터 전처리와 모델 평가를 통해 챗봇 외 머신러닝 경험 보완

<h4>위인 멘토 챗봇 | 생성형 AI·챗봇 보조 사례</h4>

- AI 교육 프로젝트 · NLP, LLM, 감정/의도 분류, 프롬프트
- 생성형 AI·대화형 서비스 지원 시 선택적으로 전면 배치

<br>

## 📚 &nbsp;EDUCATION & CERTIFICATES
- [가상융합 아카데미] 백엔드 과정_5기, 2026.05 - 2026.08(예정)
- [이스트캠프] 와썹 AI 모델 개발 과정, 2025.11 - 2026.04
- ELK 활용 빅데이터 분석서비스 플랫폼 개발자 양성 과정, 2022.03 - 2022.08
- 컴퓨터공학 전공 학점은행제, 2026.02
- 정보처리산업기사 필기 합격, 2026.03
- 정보처리기능사, 2016.09
- 컴퓨터활용능력 1급, 2019.05

<br>

## 🤝🏻 &nbsp;CONTACT

<a href="mailto:ijuju88@naver.com"><img src="https://img.shields.io/badge/-ijuju88@naver.com-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
<a href="https://github.com/mite88"><img src="https://img.shields.io/badge/-GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/></a>
<a href="https://mite88.github.io"><img src="https://img.shields.io/badge/-Blog-0A66C2?style=for-the-badge&logo=githubpages&logoColor=white"/></a>

<br>
